...
function TaskManagerPackage(){
	...
	function TaskEnvirovment(uid){
		...
		function Task(__function){
			...
			this.trigger = function(name, __context){
				...
				instance[0].trigger(name, context, self);
	...
	instance[0] = new TaskEnvirovment(0);

	instance[0].trigger = function(name, __context, __task){

		var self = this;

		var task = __task || false;

		if(!task) return this;

		var context = __context || {};

		if(globalEvents.hasOwnProperty(name)){

			globalEvents[name].forEach(function(c){

				c.call(task || self, context);
	...
	instance[0].on = function(name, __c){
		var c = __c || function(){};
